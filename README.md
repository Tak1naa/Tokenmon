...
**精灵球悬浮球**(红白 Poké Ball,常驻置顶,可拖动到任意位置,球身显示 token 用量缩写;点击时**上半球向上、下半球向下分离**,面板从**两半中间**展开,球心在屏幕上保持不动、也不左右平移——球与面板是同一个窗口,天然跟随球的位置):
...
**菜单定位**:球与面板是**同一个窗口**——打开时上半球向上、下半球向下分离,面板从两半中间展开,**球心在屏幕上保持不动、也不左右平移**(窗口呈窄长形,宽 72px ≈ 球宽,两半球吸附在面板上下缘)。X11/Windows 下窗口向上下两侧同步扩展;Wayland 不允许程序移动窗口,改为球在窗口内下移,效果近似。收起时一切复位。
