...
首次运行自动生成 `~/.config/tokenmon/config.toml`(含 api_key,权限自动 600),按你的网关编辑。改完可以在主界面点 `设置 → 重载配置` **热生效**(无需重启),或直接用 `设置 → 编辑配置` 打开编辑器:
...
| 拖动精灵球 | 移动位置(支持多屏,面板随窗口一起);拖到屏幕**左右边缘**释放即**吸附桌面**(右侧向左转 90°、左侧向右转 90°,点击后面板朝屏幕中心展开);拖离边缘解除吸附 |
| 精灵球右键 | 菜单:显示主界面 / **皮肤**(切换球样式)/ 退出 |
| 托盘图标左键 | 展开/收起面板;若悬浮球被窗口管理器关闭(如 Alt+F4),点击托盘可重新拉起(Windows 原生;GNOME 需 AppIndicator 扩展) |
| Esc / 点击精灵球 | 收起(回到精灵球) |
| 主界面 `⟳` | 手动立即刷新用量与最近对话(自动间隔见悬停提示) |
| 主界面 `详情` | 从按钮处弹出其余数据下拉 |
...
**置顶(always_on_top)**:Windows/X11 原生生效;Wayland 没有置顶协议,合成器(GNOME/KDE)会忽略该标志,状态行会提示,不影响使用。配置默认开启;也可在主界面「设置 → 窗口置顶」随时切换(本次会话有效,配置文件可持久设置)。

**菜单定位**:球与面板是**同一个窗口**——打开时上半球向上、下半球向下分离(保留原始半球形状),面板从两半中间展开,球心在屏幕上保持不动、也不左右平移;窗口呈"球 + 面板"组合形(宽 200px),外侧带**主题色描边**。X11/Windows 下窗口向上下两侧同步扩展;Wayland 不允许程序移动窗口,改为球在窗口内下移,效果近似。收起时一切复位。
