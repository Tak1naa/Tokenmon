...
**精灵球悬浮球**(红白 Poké Ball,常驻置顶,可拖动到任意位置,球身显示 token 用量缩写;点击时左右两半向两侧分离,面板从**两半中间**展开——球与面板是同一个窗口,天然跟随球的位置):

```text
    合拢态:                     打开态(两半分离,面板从中间展开):
    ╭────────╮                ◗◗◗◗ ┌────────────────────┐ ◖◖◖◖
    │  2.2M  │                ◗   ◗ │ [●] TokenMon …     │ ◖   ◖
    │━━━━━━━━│                ◗━━━◗ │ Token 用量  2.2M   │ ◖━━━◖
    │   ◎    │                ◗ ◎ ◗ │ 费用      ¥0.3755  │ ◖ ◎ ◖
...
缓存命中          1,988,736
费用              ¥0.3755
[详情 ▾] [对话 ▾]  http://127.0.0.1:8080/usage
```
...
- **皮肤系统**: 右键精灵球 → 「皮肤」菜单可切换 **精灵球 / 大师球 / 超级球 / 高级球 / 治愈球**,外观按宝可梦官方设定:大师球紫色上半 + 粉色 M 徽记,超级球蓝色上半 + 两侧红色凸起,高级球深色上半 + 黄色横纹,治愈球粉色上半 + 按钮加号;选择自动保存并同步到托盘图标
...
**菜单定位**:球与面板是**同一个窗口**——打开时两半向两侧分离,面板从两半中间展开,且**球与面板垂直居中**。X11/Windows 下窗口整体上移、球在屏幕上保持原位;Wayland 不允许程序移动窗口,球在窗口内下移实现同样效果。收起时一切复位。
